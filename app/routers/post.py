...
router = APIRouter(
    prefix="/posts",
    tags=['posts'] #grouping the routes
)
...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # conn.commit()
    new_post = models.Post(owner_id=current_user.id,**post.dict()) #convert post to dictionary
    db.add(new_post) #add to the session
    db.commit() #commit to the database
    db.refresh(new_post) #refresh the database(retieve )
...
def update_post(id: int, post: schemas.PostCreate, db: Session = Depends(get_db),  current_user: int = Depends(oauth2.get_current_user)): #ensure request comes with the right schema
